...
window.addEventListener('DOMContentLoaded', function () {
  var indexSlider = new _modules_slider_slider_main__WEBPACK_IMPORTED_MODULE_0__["default"]({
    btns: '.next',
    container: '.page'
  });
  indexSlider.render();
  var modulePageSlide = new _modules_slider_slider_main__WEBPACK_IMPORTED_MODULE_0__["default"]({
    btns: '.next',
    container: '.moduleapp',
    next: '.nextmodule',
    prev: '.prevmodule'
...
function () {
  ...
  _createClass(Difference, [{
    key: "bindTriggers",
    value: function bindTriggers(selector, items, count) {
      ...
      this.forms.forEach(function (item) {
        item.addEventListener('submit', function (e) {
          ...
          _this.postData(_this.path, formData).then(function (result) {
            ...
            setTimeout(function () {
              statusMessage.remove(); //   item.style.display = 'block';
  ...
  _createClass(VideoPlayer, [{
    ...
    value: function onPlayerStateChange(state) {
      try {
        ...
        var blockedElem = this.activeBtn.closest('.module__video-item').nextElementSibling; //глубокое клонирование параметров кнопки от предыдущего видео

        var playBtn = this.activeBtn.querySelector('svg').cloneNode(true); //Если свойства объекта state.data = 0 (воспроизведение видео завершено)

        if (state.data === 0) {
          //Если кнопка не активна
          if (blockedElem.querySelector('.play__circle').classList.contains('closed')) {
            //То разблокируем видео
            blockedElem.querySelector('.play__circle').classList.remove('closed'); //Удалим значок замочка

            blockedElem.querySelector('svg').remove(); //Назначим кнопке новые параметры

            blockedElem.querySelector('.play__circle').appendChild(playBtn); //Меняем текст рядом с кнопкой

            blockedElem.querySelector('.play__text').textContent = 'play video'; //Удаляем ненужный класс с текста и меняем стили на видео
...
function (_Slider) {
  ...
  _createClass(MainSlider, [{
    ...
    key: "showHanson",
    value: function showHanson(selector) {
      var hanson = document.querySelector(selector);
    ...
    key: "bindTriggers",
    value: function bindTriggers(switchBtn, index) {
      var _this = this;
...
function (_Slider) {
  ...
  _createClass(MiniSlider, [{
    ...
    value: function nextSlide() {
      for (var i = 1; i < this.slides.length - 1; i += 1) {
    ...
    value: function bindTiggers() {
      ...
      this.prev[0].addEventListener('click', function () {
        for (var i = _this3.slides.length - 1; i > 0; i -= 1) {
    ...
    value: function init() {
      ...
      try {
        ...
        if (this.autoplay) {
          this.autoplayStart();
          this.slides[0].parentNode.addEventListener('mousleave', function () {
            _this4.autoplayStart();
          });
          this.slides[0].parentNode.addEventListener('mousleave', function () {
            _this4.autoplayStart();
          });
